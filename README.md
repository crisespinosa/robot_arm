...
Проект состоит из двух частей и предполагает работу в **двух средах**:

- **Backend** — выполняется в **Ubuntu (Linux)**, отвечает за расчёт оптимальных траекторий и управление.
- **Unity** — используется для визуализации робота UR5e и интерактивного управления.
---

## 1. Установка и запуск (пошагово)

### 1.1 Backend (`robot_arm/`) — **Ubuntu**

Backend должен быть собран и запущен **в среде Ubuntu**, так как используется:
- компиляция C++,
- фреймворк **Drogon**,
- стандартные Linux-инструменты сборки.
...
### 2.2 Динамическая модель (трёхкратный интегратор)
...
В коде используется согласованное восстановление сопряжённых переменных через управление $u(t)$:
