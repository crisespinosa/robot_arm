 ...
 *   - Шагов управления (LQR + обратная динамика) — ядро системы управления.
 *
...
private:
    ...
    // ============================================================
    // Метрики выполнения траектории для вывода в терминал
    // ============================================================
    struct EpisodeStats {
        double sum_eq_sq{0.0};       // сумма квадратов ошибок q по всем суставам и шагам
        double sum_edq_sq{0.0};      // сумма квадратов ошибок dq по всем суставам и шагам
        double sum_u_sq_dt{0.0};     // приближённая энергия управления ∫||tau||^2 dt
        double max_abs_eq{0.0};      // максимальная абсолютная ошибка по суставам
        double last_eq_rms{0.0};     // RMS ошибки q на последнем шаге
        double last_t{0.0};          // последнее время шага
        int n_samples{0};            // число обработанных шагов
        bool summary_printed{false}; // чтобы итог печатался один раз
